    ...
    if ($rgai) {
        ...
        while ($row = $rgai->fetch_assoc()) {
          ...
          for ($i=0;$i<5;$i++) {
            ...
            $rw .= '<div class="left_td table_td" style="  width:100px;min-width:1px;text-align:center;"><img src="imgart/' . $row['ruta'.$i] . '" alt="" height="300px" width="200px">' . $row['ruta'.$i] . '</div>';
...
//recoger todas las etiquetas y meterlas en un desplegable
    ...
    while ($row = $rgeba->fetch_assoc()) {
        ...
        $re .= '<a href="vista-articulo.php?id_articulo=' . $id_articulo . '&id_etiqueta=' . $row['id_etiqueta'] . '">Eliminar</a>';
    ...
    if ($i < $aM->get_total_etiquetas()) {
        $re .= '<form name="form" method="post" action="vista-articulo.php?id_articulo='.$id_articulo.'">';
