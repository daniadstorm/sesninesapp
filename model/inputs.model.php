...
class inputsModel {
    ...
    */
    function get_input_text($id, $val, $class='', $lbl='', $placeholder='', $err_desc='', $max_length=false, $min_length=1, $allow_empty=false) {
        
        
        
        $aux_pattern = '';
        if ($min_length == 0) $aux_pattern .= '.{0}|';
        if ($min_lenght > 0) $aux_pattern .= '';
        
        //<input pattern=".{0}|.{5,10}" required title="Either 0 OR (5 to 10 chars)">
        $o  = '<div class="form-group">';
        
        if (strlen($lbl_campo) > 0) $o .= '<label>'.$lbl_campo.'</label>';
